var cones, flavors, ics, image_prepend, socket, workers;
...
angular.module('ics').controller('highscores', ["$scope", function($scope) {}]).controller('quests', ["$scope", function($scope) {}]).controller('events', ["$scope", function($scope) {}]).controller('trends', ["$scope", function($scope) {}]).config(["$sceDelegateProvider", function($sceDelegateProvider) {
  return $sceDelegateProvider.resourceUrlWhitelist(['self', 'http://static.icecreamstand.ca/**', 'https://s3.amazonaws.com/icecreamstand.com/**']);
}]).filter('img_sanitize', function() {
  return function(input) {
    return input.replace(/\W+/g, '');
  };
}).filter('number_commas', function() {
  return function(input) {
...
}).filter('item_name', function() {
  return function(input) {
    if (!input || typeof input === 'undefined') {
      return '?';
    }
    return input.toString().replace(/_/g, " ").split(' ').reverse().join(' ');
  };
...
angular.module('ics').factory('addonService', ["$http", "$q", function($http, $q) {
  var vm;
  vm = {
    addons_locked: [],
    addons_unlocked: [],
    load_addons: function() {
      ...
      vm.addons_locked = [];
      vm.addons_unlocked = [];
      vm.get_addons(user.upgrade_addon).then(function(addons) {
        var addon, i, name, topping_index;
        ...
        return df.resolve(vm.addons_unlocked);
      });
      return df.promise;
    },
    get_addons: function(level) {
      return $http.get('/addons.json?u=' + level).then(function(result) {
        return result.data;
      });
    },
    get_unlocked_addons: function() {
      return vm.addons_unlocked;
    },
    get_locked_addons: function() {
      console.log("get_locked_addons", vm.addons_locked);
      return vm.addons_locked;
    },
    set_unlocked_addons: function(unlocked) {
      return vm.addons_unlocked = unlocked;
    }
  };
  return vm;
}]);

angular.module('ics').filter('chat_sanitize', ["$sce", function($sce) {
  return function(input) {
    input = String(input).replace(/[\u00A0-\u9999<>\&]/gim, function(i) {
      return "&#" + (i.charCodeAt(0)) + ";";
    }).replace(/(https?:\/\/[^\s]*)/g, function(href) {
      var href_end, output;
      ...
      if (href_end === '.png' || href_end === '.jpg' || href_end === '.jpeg' || href_end === '.gif') {
        output = output + ("<div class='img_inline' x-href='" + href + "'>view</div>");
      }
      return output;
    }).replace(/(^| )@([^\W]*)/g, ' <div class="user_card" x-user="$2">@$2</div>').replace(/:party:/g, '<img src="' + image_prepend + '/party.png" class="party_icon" />').replace(/:easter:/g, '<img src="' + image_prepend + '/easter/chocbunny.png" class="party_icon" />');
    return $sce.trustAsHtml(input);
  };
}]);

angular.module('ics').factory('chatService', ["$http", function($http) {
  return {
    get_messages: function(message_count) {
      return $http.get('/chat.json?c=' + message_count).then(function(result) {
        return result.data;
      });
    }
  };
...
angular.module('ics').controller('flavourController', ["$scope", "upgradesService", "flavourService", "$interval", "expertiseService", "addonService", function($scope, upgradesService, flavourService, $interval, expertiseService, addonService) {
  ...
  cache_flavour_len = 0;
  cache_addon_len = 0;
  ref = $scope.cones_unlocked.length;
  for (j = 0, len = ref.length; j < len; j++) {
    i = ref[j];
    if ($scope.cones_unlocked[i] === 'babycone') {
      $scope.cones_unlocked[i] = 'baby';
    }
  ...
  $scope.update_page = function(new_page) {
    ...
    return $scope.currentPage = new_page;
  };
  $scope.expand = function() {
    return $scope.is_expanded = !$scope.is_expanded;
  };
  ...
  $scope.load_combos = function() {
    ...
    return flavourService.get_combos().then(function(raw) {
      return $scope.combos = flavourService.format_combos(raw);
    });
  };
  $scope.display_experience = function(index) {
    return expertiseService.get_level(user.flavors_sold[index]) || 0;
  };
  flavourService.load_flavours().then(function(unlocked) {
    $scope.process_unlocked(unlocked);
    return update_sell_value();
  });
  addonService.load_addons().then(function(unlocked) {
    console.log("load_addons", unlocked);
    return $scope.process_unlocked_addons(unlocked);
  });
  $scope.process_unlocked = function(unlocked) {
  ...
  $scope.process_unlocked_addons = function(unlocked) {
    if (cache_addon_len !== unlocked.length) {
      ...
      return cache_addon_len = $scope.addons_unlocked.length;
    }
  };
  ...
  return $scope.$on('cones', function(event, args) {
    if (user.cones) {
      return $scope.cones_unlocked = user.cones;
    }
  });
}]);

angular.module('ics').factory('flavourService', ["$http", "upgradesService", "$q", function($http, upgradesService, $q) {
  var vm;
  vm = {
    flavours_locked: [],
    flavours_unlocked: [],
    get_combos: function() {
      return $http.get('/combos.json').then(function(result) {
        return result.data;
      });
    },
    format_combos: function(combos) {
      var addon, flavor, i, j, len, new_combos, ref;
      new_combos = combos;
      ref = new_combos.length;
      for (j = 0, len = ref.length; j < len; j++) {
        i = ref[j];
    ...
    get_flavours: function() {
      return $http.get('/flavors.json').then(function(result) {
        return result.data;
      });
    },
...
angular.module('ics').controller('money', ["$scope", "$interval", function($scope, $interval) {
  var vm;
  vm = this;
  vm.cache = '';
  ...
  return $interval(function() {
    ...
    if (vm.new_title && vm.new_title !== vm.cache) {
      return $('title')[0].textContent = vm.new_title;
    }
  }, 500);
}]);

angular.module('ics').directive('shop', function() {
  return {
    restrict: 'E',
    replace: true,
    ...
    controller: ["$scope", "$http", "upgradesService", "flavourService", "addonService", "$rootScope", function($scope, $http, upgradesService, flavourService, addonService, $rootScope) {
      ...
      return $scope.purchase = function(type, object, amount) {
        ...
        return upgradesService.unlock(type, object._id || object.name, amount).then(function(result) {
          ...
          if (result.success === 'cone') {
            return populate_cones();
          }
        });
      };
    }]
...
$(function() {
  var shop_badge_add, shop_item_add, shop_skin_add;
  $('body').on('click', '.shop_button', function() {
    var cost, is_badge, is_skin, item;
    item = $(this).attr('x-item');
    is_skin = $(this).attr('x-is-skin') === 'true';
    is_badge = $(this).attr('x-is-badge') === 'true';
      ...
      success: function(j) {
        ...
        if (j.gamble) {
          ...
          Icecream.sync_cow(function() {
            cow_redraw();
            return update_sell_value('shopping');
          });
  ...
  shop_item_add = function(compiled, i, item) {
    return compiled + '<tr><td><img src="https://s3.amazonaws.com/icecreamstand.com/items/' + i.replace(/\s/g, '') + '.png" class="tooltip inventory_thumb" x-type="item" x-name="' + i + '/' + item.int + '/' + item.str + '/' + item.con + '/' + item.rarity + '" /> ' + i.replace(/_/g, ' - ') + '</td><td><div class="shop_button button" x-cost="' + item.cost + '" x-item="' + i + '"><span class="money_icon is_white">' + numberWithCommas(item.cost) + '</span></div></td></tr>';
  };
  shop_badge_add = function(compiled, i, item) {
    var badge;
    console.log(item);
    badge = item.badge;
    return compiled + '<tr><td><img src="https://s3.amazonaws.com/icecreamstand.com/badges/' + badge + '.png" class="inventory_thumb" x-type="badge" /> ' + i.replace(/_/g, ' - ') + '</td><td><div class="shop_button button" x-cost="' + item.cost + '" x-item="' + i + '" x-is-badge="true"><span class="money_icon is_white">' + numberWithCommas(item.cost) + '</span></div></td></tr>';
  };
  return shop_skin_add = function(compiled, i, item) {
    var flavour, req;
    ...
    return compiled + '<tr><td class="skin_td"  x-background="' + item.background + '"><img src="https://s3.amazonaws.com/icecreamstand.com/skins/' + i.replace(/\s/g, '').toLowerCase() + '.png" class="tooltip inventory_thumb thumb_wide" x-type="skin" x-name="' + i + '" /></td><td>' + i.replace(/_/g, ' - ') + '<div class="item_req">' + req + '</div>' + '</td><td><div class="shop_button button" x-cost="' + item.cost + '" x-item="' + i + '" x-is-skin="true" x-unlocked="' + (cow.skins_unlocked && cow.skins_unlocked.indexOf(i) > -1) + '"><span class="money_icon is_white">' + numberWithCommas(item.cost) + '</span></div></td></tr>';
  };
});

angular.module('ics').factory('socketFactory', ["$rootScope", function($rootScope) {
  return {
    on: function(eventName, callback) {
      return typeof socket !== "undefined" && socket !== null ? socket.on(eventName, function(args) {
    ...
    emit: function(eventName, data, callback) {
      return typeof socket !== "undefined" && socket !== null ? socket.emit(eventName, data, function(args) {
        return $rootScope.$apply(function() {
          if (callback) {
            return callback(args);
          }
...
bind_sockets = function() {
  socket.on('trend', function(msg) {
    var err, flavour, flavours, j, k, len, trending_bonus, trending_flavor;
    j = msg.trend;
    trending_flavor = j.flavour_id;
    flavours = [];
    try {
      for (k = 0, len = flavours.length; k < len; k++) {
        flavour = flavours[k];
        if (flavour._id === trending_flavor) {
          flavour = flavors[i];
          break;
        }
      }
    ...
    if (flavour) {
      ...
      if ($('.currently_trending[x-id="' + trending_flavor + '"]').length === 0) {
        $('.trending .currently_trending_container').html('<span class="currently_trending tooltip" x-type="base" x-id="' + trending_flavor + '" id="' + j.flavour_name.replace(/\s+/g, '') + '"><img src="' + image_prepend + '/flavours/thumb/' + j.flavour_name.replace(/\s+/g, '') + '.png.gz" class="trending_img active" /></span></span>');
      } else {
        $('.trending #trend_bonus').text(trending_bonus).show();
      }
    } else {
      $('.trending .currently_trending_container').html('<span class="currently_trending tooltip" x-type="base" x-id="' + trending_flavor + '" id="' + j.flavour_name.replace(/\s+/g, '') + '"><img src="' + image_prepend + '/flavours/thumb/' + j.flavour_name.replace(/\s+/g, '') + '.png.gz" class="trending_img active trending_locked" /></span></span>');
    }
  ...
  socket.on('epic/aoc/log', function(msg) {
    ...
    if ($('.aoc_log_msg').length > 10) {
      return $('.aoc_log_msg:first').remove();
    }
  });
  socket.on('update', function(msg) {
    ...
    if (msg.refresh) {
      game_working = false;
      $('body').append('<div class="white_overlay"><strong>Already signed in (or the server just went down)<br><br><small>Refreshing in 10 seconds</small></strong>' + '<i class="fa fa-times"></i></div>');
      $('.white_overlay i').click(function() {
        return $('.white_overlay').fadeOut(250, function() {
          $(this).remove();
          return game_working = true;
        });
      });
      setTimeout(function() {
        if (!game_working) {
    ...
    if (msg.expertise && msg.expertise_position) {
      return user.flavors_sold[msg.expertise_position] = msg.expertise;
    }
  });
  socket.on('alert', function(msg) {
    ...
    return alert(msg.message, msg.title);
  });
  socket.on('leave', function(msg) {
    ...
    if (friend) {
      ...
      return $('.friends_counter span#count')[0].textContent = $('.friends_list_online > user, .friends_list_away > user').length + '/' + $('.friends_counter span#count').attr('x-length');
    }
  });
  return socket.on('typing', function(msg) {
    typing_cache[msg._id] = new Date();
    setTimeout(function() {
      if (typing_cache[msg._id] < new Date() - 900) {
        return $('.is_typing[x-id="' + msg._id + '"]').remove();
      }
    }, 1000);
    if ($('.is_typing[x-id="' + msg._id + '"]').length === 0) {
      return $('#typing').append('<span class="is_typing" x-id="' + msg._id + '">' + msg.name + ' is typing.</span>');
    }
  });
};
...
angular.module('ics').controller('upgrades', ["$scope", "upgradesService", "$rootScope", function($scope, upgradesService, $rootScope) {}]);

angular.module('ics').factory('upgradesService', ["$http", function($http) {
  var vm;
  vm = {
    ...
    order_by_id: function(raw, sorted) {
      var i, j, k, l, len, new_array, ref, ref1;
      if (!sorted) {
