...
$(document).ready(function () {
    ...
    $('body').on('click', '.flavor .flavor_tab', function () {
        ...
        if (xid == 'main_combo') {
            ...
            $('.flavor').append('<div class="inner_flavor_container" id="main_franken">' +
                ...
                '<div class="col_3 franken_center"><b>&lt; - - - - - &gt;</b><p>Select your flavours<br><br></p><button class="button_green">Combine!</button></div><div class="col_3 franken_right"><div class="option_wrapper"></div></div></div>');
        }
        init_canvas();
    });
    ...
    $('body').on('mouseover', '.tooltip', function () {
        ...
        if (xtype == 'sales_cart') {
            $('.hovercard').html('<div class="hover_title">' + __('Cart') + '<span class="level">' + user_me.carts + '</span></div><p>' + __('Automatically sells 1 ice cream every') + ' ' + (5 - (user_me.upgrade_machinery*0.25)) + ' ' + __('seconds') + '</p><p class="flavor_text">' + __('Workers sell your top row. Can be leveled up to 1000.') + '</p>');
        } else if (xtype == 'sales_employee') {
            $('.hovercard').html('<div class="hover_title">' + __('Employees') + '<span class="level">' + user_me.employees + '</span></div><p>' + __('Automatically sells 2 ice cream every') + ' ' + (5 - (user_me.upgrade_machinery*0.25)) + ' ' + __('seconds') + '</p><p class="flavor_text">' + __('Workers sell your top row. Can be leveled up to 1000.') + ' ' + __('Unlocks after you have 10 Carts.') + '</p>');
        } else if (xtype == 'sales_truck') {
            $('.hovercard').html('<div class="hover_title">' + __('Truck') + '<span class="level">' + user_me.trucks + '</span></div><p>Each truck automatically sells 3 ice creams every ' + (5 - (user_me.upgrade_machinery*0.25)) + ' seconds</p><p class="flavor_text">Workers sell your top row. Can be leveled up to 1000. Unlocks after you have 25 Employees.</p>');
        } else if (xtype == 'sales_robot') {
            $('.hovercard').html('<div class="hover_title">' + __('Robot') + '<span class="level">' + user_me.robots + '</span></div><p>Each robot automatically sells 5 ice creams every ' + (5 - (user_me.upgrade_machinery*0.25)) + ' seconds</p><p class="flavor_text">Workers sell your top row. Can be leveled up to 1000. Unlocks after you have 50 Trucks.</p>');
        } else if (xtype == 'sales_rocket') {
            $('.hovercard').html('<div class="hover_title">' + __('Rocket') + '<span class="level">' + user_me.rockets + '</span></div><p>Each rocket automatically sells 10 ice creams every ' + (5 - (user_me.upgrade_machinery*0.25)) + ' seconds</p><p class="flavor_text">Workers sell your top row. Can be leveled up to 1000. Unlocks after you have 100 Robots.</p>');
        } else if (xtype == 'sales_alien') {
            $('.hovercard').html('<div class="hover_title">' + __('Alien') + '<span class="level">' + user_me.aliens + '</span></div><p>Each alien automatically sells 15 ice creams every ' + (5 - (user_me.upgrade_machinery*0.25)) + ' seconds</p><p class="flavor_text">Workers sell your top row. Can be leveled up to 1000. Unlocks after you have 250 Rockets.</p>');
            ...
            if (item == 'hay') stats = 'This item <b>increases Happiness</b> by 10. Drag this onto your cow.';
            if (item == 'rock') stats = 'This item <b>decreases Happiness</b> by 40. Drag this onto your cow.';
            $('.hovercard').html('<div class="hover_title" style="padding-left: 40px;">' + name + '</div><img src="' + image_prepend + '/items/' + item_split[0].replace(/ /g, '') + '.png" class="tooltip_item" />' + stats + 
                rarity + special);
            $('.hovercard').css('z-index',12);
            ...
            success: function (j) { 
                ...
                _gaq.push(['_trackEvent', 'Quest', 'Completed Quest', user_me.quests.length]);
                    ...
                    if (id == '526745240fc3180000000002') {
                        ...
                        alert('<img class="quest_princess" src="' + image_prepend + '/princess_quest.png">Huzzah! Successfully completed the quest, earning + $1.00 when there is an add-on event.<br /><br /><b>The next quest will unlock in a couple minutes.</b>', 'Quest Complete'); 
                    }
                    main('quest');
    ...
    $('body').on('click', '.icecream', function (e) {
        ...
        $('.cube_infobar').remove();
        $('#canvas_sales').show().before('<div class="cube_infobar"><span class="infocube_time">00:00</span> <span class="money_icon infocube_money">0</span> <span class="infocube_multiplier"></span></div>');
    ...
    function cubebar_end() {
        ...
        socket.emit('accumulation', { a: cache_cube_money, t: (new Date() - cache_cube_time) / 1000, is_first_win: cache_first_win_avail });
        setTimeout(function() {
            $('.cube_infobar').remove();
            $('#canvas_sales').hide();
            canvas_cache_sales = [];
    ...
    function cube_new(is_rock) {
        ...
        if (is_rock) {
            var rand = Math.random() < 0.025;
            canvas_cache_sales.push( [ (rand)?  100 : 150, new_x, new_y, 8, (rand)? 9 : 8] );
            return;
        } else {
    ...
    $('body').on('mousedown touchend', '#canvas_sales', function (e) {
        ...
        for (var i = 0; i < cube_len; i++) {
            ...
            if (within_z(x, x2, y, y2, 18)) {
                ...
                icecream_mousedown(num_of_clicks);
    ...
    $('body').on('click', '.view_credits', function () {
        $.ajax({
            ...
            success: function (j) {
                ...
                alert( 'Special thanks to:<br>' + compiled, 'Credits');
            }
        })
        return false;
    });
...
function bind_sockets() {
    socket.on('chat message', function(msg){
        ...
        if (msg.party && msg.party == user_me.name) {
            main('party', function () {
    ...
    socket.on('update', function(msg){
        ...
        if (msg.expertise) {
            var flavour_position = user_me.flavors.indexOf(user_me.last_flavor);
            user_me.flavors_sold[flavour_position] = msg.expertise;
        }
    });
    socket.on('alert', function(msg){
        if (msg.error) {
            $('.inline-message').remove();
            $('body').append('<h4 class="inline-message" id="animations">Ack Ack!<br /><small>' + msg.error + '</small></h4>'); 
            return;
        }
...
function change_room(room) {
    ...
    $('.chat_rooms').append('<div class="hidden"></div>');
    ...
    if (cache_rooms_avail.length === 0) {
        ...
        if (user_me.badges && user_me.badges.indexOf(1) !== -1) {
            cache_rooms_avail.push('donor lounge');
        }
    }
...
function epic_load() {
    $('.trending_and_events.main_container').after('<div class="epic main_container"><h4>Winter Event</h4>' +
        ...
        '<span class="link_underline" x-link="invite">Invite a friend here</span> and they will automatically join your team. Current bonus: 10X - Winter Event ends at midnight on December 31st (new years).</p><div class="epic_progress_container">' +
...
function epic_interval() {
    ...
    $.ajax({
        ...
        success: function (epics) {
            ...
            $('.team_epic_collected')[0].textContent = (epics[0]._id == user_me.epic_id)? numberWithCommas(epics[0].total) : numberWithCommas(epics[1].total);
        }
    });
}
...
function update_worker_tiers() {
    ...
    $('#unlock_autopilot .unlock_text .cost').text(numberWithCommas( (250 * (user_me.upgrade_autopilot+1) * (user_me.upgrade_autopilot+1)) + parseInt(Math.pow(1.05, user_me.upgrade_autopilot*2)) ));
    $('#unlock_autopilot .sale_level').text(user_me.upgrade_autopilot);
    $('#unlock_coldhands .unlock_text .cost').text(numberWithCommas( 250 * parseInt( Math.pow(user_me.upgrade_coldhands*2,1.6)) ));
    $('#unlock_coldhands .sale_level').text(user_me.upgrade_coldhands);
    $('#unlock_cart .unlock_text .cost').text(numberWithCommas( (25 + (Math.pow(user_me.carts, 2) / 4)) ));
    $('#unlock_cart .sale_level').text(user_me.carts);
    $('#unlock_employee .unlock_text .cost').text(numberWithCommas(  (150 + (user_me.employees * 100)) ));
    $('#unlock_employee .sale_level').text(user_me.employees);
    $('#unlock_truck .unlock_text .cost').text(numberWithCommas(  (1000 + (user_me.trucks * user_me.trucks * 50)) ));
    $('#unlock_truck .sale_level').text(user_me.trucks);
    $('#unlock_robot .unlock_text .cost').text(numberWithCommas( (5000 + (user_me.robots * user_me.robots * 100)) ));
    $('#unlock_robot .sale_level').text(user_me.robots);
    $('#unlock_rocket .unlock_text .cost').text(numberWithCommas( (50000 + (user_me.rockets * user_me.rockets * 500)).toFixed(2) ));
    $('#unlock_rocket .sale_level').text(user_me.rockets);
    $('#unlock_alien .unlock_text .cost').text(numberWithCommas( (500000+(30 * Math.pow(user_me.aliens,2.5))).toFixed(2) ));
    ...
    $.ajax({
        ...
        success: function (j) {
            ...
            if (user_me.is_guest && user_me.total_gold > 100) {
                ...
                '<small>' + __('Click here to secure your account and set a username and password.') + '</small></h4>'); 
            }
            if (!user_me.email && user_me.total_gold > 20000) {
            ...
            if (!user_me.email_verified && user_me.email && user_me.total_gold > 20000) {
...
function load_message(msg) {
    ...
    var construct = $('<span />', { html: msg.text.replace(/[\u00A0-\u9999<>\&]/gim, function(i) {
        return '&#'+i.charCodeAt(0)+';';
...
function icecream_mousedown(amount) {
        ...
        var floater = $('<div />', {
            ...
            'html': '<img src="http://static.icecreamstand.ca/coins2.svg" class="money_float" /><b>' + numberWithCommas( (cache_sell_float).toFixed(2) ) + '</b>'
        });
        $('.icecream').append(floater);
        $(floater).css('color', '#' + color_pool[Math.floor( Math.random() * color_pool.length )]).animate({ 'top': '0' }, 500, 'easeInQuint', function () { $(this).remove() });
...
return {
    ...
    canvas_icecream_sales: function() {
        ...
        for (var i = 0; i < clen; i++) {
            ...
            canvas_sales_context.globalAlpha= (c[0] <= 150)? 0.75 : 1;
    ...
    first_time_party: function() {
        ...
        if (user_me.party_until && new Date(user_me.party_until) > new Date()) {
            ...
            $('.footer').prepend('<div class="dunce" x-type="party">While you are partying you can invite others to party. Invite people to party by typing /party player_name.<br>Time left: ' + time_diff.toFixed(1) + ' minutes.</div>');
        ...
        if (cow && cow.name) {
            ...
            $.ajax({
                ...
                success: function (cows) {
                    ...
                    if (!cow || !cow._id) {
                        $('.cow').prepend('<div class="cow_body adopt_cow tooltip" x-loading="false" x-skin="baby" x-type="adopt_cow"></div>');
                        $('.adopt_cow').click(function () {
                            if (cow && cow.name) return false;
                            if ( $(this).attr('x-loading') == 'true') return false;
                            $(this).attr('x-loading', 'true');
                            $.ajax({
                                url: '/unlock',
                                data: { type: 'adopt_cow' },
                                dataType: 'json',
                                ...
                                success: function(j) {
                                    ...
                                    $('.cow')[0].textContent = '';
                                    Icecream.sync_cow();
                                    alert('<p><b>You have adopted a cow!</b> Feed cows hay to improve their happiness.</p>' +
                                        '<p>A happy cow will give you a bonus to ice cream production. Your cow will become an adult once it hits level 1, allowing you to equip items.</p>', 'Adopted!');
                                }
                            });
                            return false;
                        });
        ...
        $.ajax({
            ...
            success: function (j) {
                ...
                if (j.time) {
                    ...
                    var diff = new Date(j.time) - new Date();
                    setTimeout(function () { Icecream.get_quest('get_quest') }, diff + 200);
                    console.log('rechecking new quests in ' + (diff + 200));
                    return;
                }
...
quests_timer: function () {
    ...
    var diff = new Date(user_me.next_quest_at) - new Date();
    var minutes = Math.floor(diff / 60000);
    $('.quests.main_container h4 > .lang')[0].textContent = 'Quests - Next quest unlocks in ' + minutes + ' ' + ((minutes == 1)? 'minute' : 'minutes');
},
get_quests: function() {
    console.log('getting quests');
    if (user_me.quests.length == 0) {
        ...
        if (user_me.gold > 10) Icecream.get_quest();
        return;
    }
    if (user_me.next_quest_at && new Date(user_me.next_quest_at) > new Date()) {
        var diff = new Date(user_me.next_quest_at) - new Date();
        Icecream.quests_timer();
        setTimeout(function () { Icecream.get_quest() }, diff + 200);
        console.log('rechecking new quests in ' + (diff + 200));
    } else {
        Icecream.get_quest();
    }
...
update_trending: function() {
    ...
    $.ajax({
        ...
        success: function (j) {
            ...
            $('.trend_title').text( __(j.flavour_name) );
            if (flavour) {
...
get_tutorial: function() {
    ...
    if (user_me.tutorial == 1) {
        ...
        '<b>' + __('When you are ready') + '</b> ' + __('research new flavours, add-ons, and adopt your first cow under Upgrade') + '<div class="button next_tutorial button_green">' + __("Let's Start") + '</div><div class="triangle-right"></div></div>');
    }
    if (user_me.total_gold > 500 && user_me.tutorial == 2) {
        ...
        '<div class="clearfix"></div><div class="button next_tutorial button_red">' + __('No thanks') + '</div><div id="invite" x-link="invite" class="button next_tutorial button_green">' + __('Sure') + '</div></div>');
    }
