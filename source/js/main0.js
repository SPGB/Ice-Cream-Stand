...
$(document).ready(function () {
            ...
            success: function (j) { 
                ...
                _gaq.push(['_trackEvent', 'Quest', 'Completed Quest', user_me.quests.length]);
                    ...
                    if (id == '526745240fc3180000000002') {
                        ...
                        alert('<p>You have unlocked chat!</p><p>Please be considerate of others and the Ice Cream community. You can enable chat in <i class="fa fa-cog"></i> settings at the bottom left.</p>', 'Chat Unlocked');
                        $.ajax({
                                url: '/unlock',
                                data: { type: 'adopt_cow' },
                                dataType: 'json',
    ...
    function cube_new(is_rock) {
        var new_x = 112, new_y = 90;

        if (cache_sell_float_num > 3) {
            do {
                new_x = 35 + (Math.random() * 140);
                new_y = 35 + (Math.random() * 105);
        ...
        if (is_rock) {
            ...
            canvas_cache_sales.push( [ (rand)?  100 : 150, new_x, new_y, 16, (rand)? 8 : 9] );
            return;
        } else {
            var progress = Math.floor(cache_sell_float_num / 10);
            if (progress > 7) progress = 7;
            canvas_cache_sales.unshift( [100, new_x, new_y, 16, progress] );
            if (progress > 2) {
                cube_new(true);
            }
            if (progress > 5) {
...
function init_canvas() {
    ...
    if (!canvas_cache_sale_cold) {
        ...
        canvas_cache_sale_cold.src = image_prepend + '/icecube_sprite.png';
    }
...
return {
    ...
    canvas_icecream_sales: function() {
        ...
        for (var i = 0; i < clen; i++) {
            var c = canvas_cache_sales[i];
            canvas_sales_context.clearRect(c[1] - 30 - 2, c[2] - 30 - 2, 65, 65);
        }
        for (var i = 0; i < clen; i++) {
            var c = canvas_cache_sales[i];
    ...
    sync_chat: function() {
        if (!$('.chat.main_container').is(':visible') || !game_working || user_me.chat_off) { 
...
get_tutorial: function() {
    ...
    if (user_me.total_gold > 50 && user_me.tutorial == 3) {
        ...
        '<b>' + __('Research powerful Ice Cream flavours and add-ons here. Hire workers, or get upgrades.') + '</b><div class="button next_tutorial button_green">' + __("Let's Start") + 
